...
/**
 * Middleware to protect routes, ensuring a valid JWT is provided.
 * Attaches user (id, role, subscriptionId, tenantAdminId, etc.) to req.user.
 */
const protect = async (req, res, next) => {
    let token;
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
        try {
            ...
            // Attach the necessary user properties to the request for subsequent logic
            req.user = {
                id: user._id,
                role: user.role,
                // --- FIX APPLIED HERE: Including subscription and tenant data ---
