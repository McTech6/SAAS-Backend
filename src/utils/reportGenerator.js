...
const getStatusInfo = (selectedValue) => {
    const raw = (selectedValue === undefined || selectedValue === null) ? '' : String(selectedValue).trim().toLowerCase();
    const color = '#014f65'; // Primary color
    return { label: raw || 'N/A', color: color };
};

/**
 * Build TOC without numbering
 */
const buildToc = (templateStructure) => {
    if (!Array.isArray(templateStructure) || templateStructure.length === 0) return '<p>(No content)</p>';
    let tocHtml = '<ul class="toc-root">';
...
const generateReportHtml = (auditInstance = {}) => {
    ...
    // ---- Table of contents removed numbering ----
    const introductionText = `
        <p class="justify-text static-text">When we speak about Cyber, Information, and IT Security...</p>
    `;
    ...
    <head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Lexend:wght@400;700&display=swap" rel="stylesheet">
        <style>
            body { font-family: 'Arial', Helvetica, sans-serif; font-size: 14pt; color: #2c3e50; margin: 0; -webkit-print-color-adjust: exact; }
            .container { padding: 0.35in; box-sizing: border-box; }
            h2, h3 { font-family: 'Lexend', sans-serif !important; color: #014f65; }
            h2 { font-size: 26pt; }
            h3 { font-size: 20pt; }
            .header-spacing { margin-top: 25px; margin-bottom: 16px; }
            .justify-text { text-align: justify; line-height: 1.5; }
            .toc-root { list-style: none; padding-left: 0; }
            .toc-root li { margin: 4px 0; }
            .toc-root a { text-decoration: none; color: #003340; }
            .question-block { margin-bottom: 6px; padding: 6px 10px; background: #fafafa; border: 1px solid #eee; border-radius: 4px; }
            .question-header { display: flex; align-items: flex-start; margin-bottom: 2px; border-left: 3px solid; padding-left: 10px; }
            .question-title { font-size: 11pt; margin: 0; }
            .answer-row, .comment, .recommendation, .evidence { font-size: 11pt; margin-top: 4px; }
            .comment { background: #e6f7f6; border-left: 3px solid #014f65; }
            .recommendation { background: #f0f8ff; border-left: 3px solid #014f65; }
            .evidence { background: #fff8e6; border-left: 3px solid #014f65; }
            .env td { font-size: 12pt; border: 1px solid #e6e6e6; padding: 5px 8px; }
            /* Page numbering */
            @media print {
                body { counter-reset: page; }
                .page { page-break-after: always; position: relative; }
                .page-number::after { content: counter(page) " / " counter(pages); position: absolute; bottom: 10px; right: 20px; font-size: 12pt; color: #333; }
            }
        </style>
    </head>
    <body>
