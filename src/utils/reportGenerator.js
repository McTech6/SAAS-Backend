 ...
 */
const buildToc = (sections) => {
    if (!Array.isArray(sections) || sections.length === 0) return '';
    let html = '<ul>';
    sections.forEach((section, sIdx) => {
        const secId = `sec-${sIdx}`;
        html += `<li><a href="#${secId}">${escapeHtml(section.name || '')}</a>`;
        if (Array.isArray(section.subSections) && section.subSections.length > 0) {
            html += '<ul>';
            section.subSections.forEach((sub, subIdx) => {
                const subId = `sec-${sIdx}-sub-${subIdx}`;
                html += `<li><a href="#${subId}">${escapeHtml(sub.name || '')}</a></li>`;
            });
            html += '</ul>';
        }
        html += '</li>';
...
/**
 * Generates full audit report HTML
 */
const generateReportHtml = (auditInstance = {}) => {
    const company = auditInstance.company || {};
    const template = auditInstance.template || {};
    ...
    const reportDate = formatDate(new Date());
    const auditDateRange = (auditInstance.startDate || auditInstance.endDate)
        ? `${formatDate(auditInstance.startDate)} - ${formatDate(auditInstance.endDate)}`
        : '';

    const auditorLines = auditorsToDisplay.map(u => `${escapeHtml(u.firstName || '')} ${escapeHtml(u.lastName || '')} (${escapeHtml(u.email || '')})`).join('<br/>') 
    ...
    templateStructure.forEach((section, sIdx) => {
        const secId = `sec-${sIdx}`;
        mainHtml += `<div class="section" id="${secId}"><h2>${escapeHtml(section.name || '')}</h2>`;
        if (section.description) mainHtml += `<p>${escapeHtml(section.description)}</p>`;
        (section.subSections || []).forEach((subSection, ssIdx) => {
            const subId = `sec-${sIdx}-sub-${ssIdx}`;
            mainHtml += `<div class="subsection" id="${subId}"><h3>${escapeHtml(subSection.name || '')}</h3>`;
            if (subSection.description) mainHtml += `<p>${escapeHtml(subSection.description)}</p>`;

            (subSection.questions || []).forEach((question) => {
                const resp = responses.find(r => r.questionId?.toString() === question._id?.toString()) || {};
                const answerText = resp.selectedValue ?? '';
                ...
                mainHtml += `<div class="question-block">
    ...
    const env = company.examinationEnvironment || {};
    const envHtml = ` 
        <table class="env">
            <tr><td><strong>Locations</strong></td><td>${escapeHtml(String(env.locations || ''))}</td></tr>
            <tr><td><strong>Employees</strong></td><td>${escapeHtml(String(env.employees || ''))}</td></tr>
            <tr><td><strong>Clients (total)</strong></td><td>${escapeHtml(String(env.clients?.total || ''))}</td></tr>
            <tr><td><strong>Clients (managed)</strong></td><td>${escapeHtml(String(env.clients?.managed || ''))}</td></tr>
            <tr><td><strong>Clients (unmanaged)</strong></td><td>${escapeHtml(String(env.clients?.unmanaged || ''))}</td></tr>
            <tr><td><strong>Industry</strong></td><td>${escapeHtml(env.industry || '')}</td></tr>
            <tr><td><strong>Physical servers</strong></td><td>${escapeHtml(String(env.physicalServers || ''))}</td></tr>
            <tr><td><strong>VM servers</strong></td><td>${escapeHtml(String(env.vmServers || ''))}</td></tr>
            <tr><td><strong>Firewalls</strong></td><td>${escapeHtml(String(env.firewalls || ''))}</td></tr>
            <tr><td><strong>Switches</strong></td><td>${escapeHtml(String(env.switches || ''))}</td></tr>
            <tr><td><strong>Mobile working</strong></td><td>${env.mobileWorking ? 'Yes' : ''}</td></tr>
            <tr><td><strong>Smartphones</strong></td><td>${env.smartphones ? 'Yes' : ''}</td></tr>
        </table>
    `;
...
<head>
<meta charset="utf-8">
<title>Audit Report - ${escapeHtml(company.name || '')}</title>
<style>
    body { font-family: Arial, sans-serif; font-size: 12pt; color: #2c3e50; margin: 0; }
    .container { padding: 30px; }
    .cover { text-align: center; }
    .logo { max-width: 140px; margin-bottom: 8px; }
    h1, h2, h3 { color: #3f51b5; }
    table.env { width: 100%; border-collapse: collapse; margin: 15px 0; }
    table.env td { border: 1px solid #ddd; padding: 6px; }
    .question-block { padding: 8px; background: #fafafa; border: 1px solid #eee; border-radius: 4px; margin-bottom: 10px; }
    .comment { background:#e6f7f6; padding:6px; border-left:3px solid #3f51b5; margin-top:6px; }
    .summary { background:#f6f6f6; padding:8px; border-radius:4px; margin-bottom: 6px; }
    a { color: #1a237e; }
...
<body>
    <div class="container cover">
        ...
        <p><strong>Report Date:</strong> ${escapeHtml(reportDate)}</p>
        <p><strong>Audit Date:</strong> ${escapeHtml(auditDateRange)}</p>
        <p><strong>Auditor(s):</strong><br/>${auditorLines}</p>
        <p><strong>For:</strong> ${escapeHtml(company.name || '')} — ${escapeHtml(contactName)} (${escapeHtml(contactEmail)})</p>
    </div>
    ...
    <div class="container page-break">
        <h2>Executive Summary</h2>
        <p>Overall compliance score: <strong>${overallScore.toFixed(2)}</strong></p>
        ${summariesHtml}
    </div>
