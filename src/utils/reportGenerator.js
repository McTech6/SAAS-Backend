...
const LOGO_URL =
  'https://res.cloudinary.com/dcviwtoog/image/upload/v1765422490/1_BKGS_Consulting_boqy3g.png';
...
const getStatusInfo = (selectedValue) => {
  ...
  if (value.includes('no') || value.includes('not'))
    return { label: selectedValue, color: '#ef4444' };

  if (value.includes('partial'))
    return { label: selectedValue, color: '#f59e0b' };

  if (value.includes('yes') || value.includes('implemented'))
    return { label: selectedValue, color: '#16a34a' };

  return { label: selectedValue, color: '#a3a3a3' };
};

/** TOC — same structure as original commented version */
const buildToc = (structure) => {
  let html = '<ul class="toc-root">';
  structure.forEach((s, i) => {
    html += `<li><a href="#sec-${i}">${escapeHtml(s.name)}</a>`;
    if (s.subSections?.length) {
      html += '<ul>';
      s.subSections.forEach((ss, j) => {
        html += `<li><a href="#sec-${i}-sub-${j}">${escapeHtml(
          ss.name
        )}</a></li>`;
      });
      html += '</ul>';
...
const generateReportHtml = (auditInstance = {}) => {
  console.log('[REPORT DATA]', auditInstance);

  const company = auditInstance.company || {};
  const template = auditInstance.template || {};
  const responses = auditInstance.responses || [];
  const structure = auditInstance.templateStructureSnapshot || [];
  const examinationEnvironment =
    auditInstance.examinationEnvironment || company.examinationEnvironment || {};

  console.log('[ENVIRONMENT]', examinationEnvironment);

  const overallScore =
    typeof auditInstance.overallScore === 'number'
      ? Math.round(auditInstance.overallScore)
      : 0;

  console.log('[SCORE]', overallScore);

  const start = formatDate(auditInstance.startDate);
  const end = formatDate(auditInstance.endDate);
  const examRange = start && end ? `${start} - ${end}` : 'N/A';

  const reportDate = formatDate(new Date());

  const examiner =
    auditInstance.auditorsToDisplay?.[0] || auditInstance.createdBy || {};

  const examinerName = `${examiner.firstName || ''} ${
    examiner.lastName || ''
  }`.trim();

  const examinerEmail = examiner.email || '';
  ...
  <div class="container cover-page">
    <div class="cover">
      ...
      <h1>${escapeHtml(template.name || 'Assessment')}</h1>
      <h2>REPORT</h2>

      <p><strong>Report Date:</strong> ${reportDate}</p>
      <p><strong>Examination’s date:</strong> ${examRange}</p>

      <p><strong>Examiner:</strong> ${escapeHtml(examinerName)}</p>
      <p><strong>E-Mail:</strong> ${escapeHtml(examinerEmail)}</p>

      <p style="margin-top:20px"><strong>For</strong></p>
      <p><strong>${escapeHtml(company.name)}</strong></p>
      <p>${escapeHtml(company.contactPerson?.name)}</p>

      <p class="justify-text">
        The strength of your defense lies in knowing and understanding your
        vulnerabilities. This assessment provides you with the information you
        need to create a secure environment in your company.
      </p>

      <p><em><strong>“You can only protect what you know.”</strong></em></p>
    </div>
  </div>`;
  ...
  <p class="justify-text">
    This report provides a comprehensive overview of the Information, IT and
    cybersecurity posture for <strong>${escapeHtml(
      company.name
    )}</strong> based on the "<strong>${escapeHtml(template.name)}</strong>".
  </p>
  <p class="justify-text">
    The assessment covered key areas including Information Security Policies,
    Access Control, Physical Security, Mobile & Remote Working, Awareness,
    Compliance & Legal Requirements and other critical domains as defined in the
    selected template.
  </p>
  <p class="justify-text">
    Overall, the assessment indicates a compliance score of
    <strong>${overallScore}%</strong>.
  </p>
  <p class="justify-text">
    It is crucial to address identified areas of non-compliance and implement
    recommended remediation actions to strengthen the overall security posture
    and ensure continuous adherence to best practices.
  </p>
  `;
  ...
  <p>Name: ____________ Organization: BKGS Consulting Date: ____________</p>
  <p>Signature:</p>

  <br/><br/>

  <p><strong>Consulted Company Representative:</strong></p>
  <p>Name: ____________ Organization: ${escapeHtml(
    company.name
  )} Date: ____________</p>
  `;
  ...
  const thankYou = `
  <p class="justify-text">
    We are committed to enhancing your organization's security posture and
    ensuring compliance in an ever-evolving threat landscape.
  </p>
  <p class="justify-text">
    For further discussions or to schedule a follow-up consultation, please
    contact your partner: <strong>${escapeHtml(
      examinerName
    )}, ${escapeHtml(examinerEmail)}</strong>
  </p>
  <h3 style="text-align:center">
    "Improvement begins with assessment and assessment begins with the right
    questions"
  </h3>
  `;
  ...
  <body>
    ${cover}
    <div class="page-break"><h2>Table of Contents</h2>${buildToc(structure)}</div>
    <div class="page-break"><h2>Executive Summary</h2>${executiveSummary}</div>
    <div class="page-break">${handover}</div>
    <div class="page-break">${thankYou}</div>
  </body>
  </html>`;
};
