...
const DICT = {
  ...
  // ---- preface / disclaimer ---------------------------------
  PREFACE_TEXT: {
    EN: `<p>The <strong>ISARION (Information Security Assessment Evolution) - Report</strong> has been developed to provide organizations with a structured and independent assessment of their information security posture. The aim is not only to identify technical issues, but to create a holistic view that combines <strong>technology, organization, and people</strong>.<p>This report is designed as a practical tool for decision-makers at all levels. Whether you have a technical background or not, the findings and recommendations are presented in a way that allows you to clearly understand where strengths lie, where risks exist, and what steps can be taken to achieve sustainable improvement.<p>Our mission is to support organizations in treating cybersecurity as an integral part of their business strategy and daily operations—helping to build trust, ensure compliance, and strengthen resilience in an ever-changing digital world.`,
    FR: `<p>Le <strong>rapport ISARION (Information Security Assessment Evolution)</strong> a été développé pour fournir aux organisations une évaluation structurée et indépendante de leur posture de sécurité de l'information. L'objectif n'est pas seulement d'identifier les problèmes techniques, mais de créer une vue d'ensemble qui combine <strong>technologie, organisation et personnes</strong>.<p>Ce rapport est conçu comme un outil pratique pour les décideurs de tous niveaux. Que vous ayez ou non un background technique, les constats et recommandations sont présentés de manière à vous permettre de comprendre clairement où se situent les forces, où existent les risques et quelles mesures peuvent être prises pour atteindre une amélioration durable.<p>Notre mission est de soutenir les organisations pour qu'elles traitent la cybersécurité comme une partie intégrante de leur stratégie d'affaires et de leurs opérations quotidiennes – afin de construire la confiance, garantir la conformité et renforcer la résilience dans un monde numérique en constante évolution.`,
    DE: `<p>Der <strong>ISARION (Information Security Assessment Evolution) - Bericht</strong> wurde entwickelt, um Organisationen eine strukturierte und unabhängige Bewertung ihrer Informationssicherheitsposition zu bieten. Ziel ist es nicht nur, technische Probleme zu identifizieren, sondern eine ganzheitliche Sicht zu schaffen, die <strong>Technologie, Organisation und Menschen</strong> kombiniert.<p>Dieser Bericht ist als praktisches Werkzeug für Entscheidungsträger aller Ebenen konzipiert. Ob Sie nun technisches Fachwissen mitbringen oder nicht – die Feststellungen und Empfehlungen werden so dargestellt, dass Sie klar verstehen, wo Stärken liegen, wo Risiken bestehen und welche Schritte unternommen werden können, um nachhaltige Verbesserungen zu erzielen.<p>Unsere Mission ist es, Organisationen dabei zu unterstützen, Cybersicherheit als integralen Bestandteil ihrer Geschäftsstrategie und ihres täglichen Betriebs zu behandeln – um Vertrauen aufzubauen, Compliance sicherzustellen und Resilienz in einer sich ständig verändernden digitalen Welt zu stärken.`
  },

  DISCLAIMER_TEXT: {
    EN: `<p>This report is based on the information, data, and evidence made available during the assessment process. While every effort has been made to provide accurate and reliable findings, the results and recommendations are limited to the scope of the assessment and the time of its execution.<p>The report should not be considered a guarantee against future risks or incidents. Security threats evolve constantly, and continuous monitoring, improvement, and adaptation remain essential.<p>The assessor and assessing organization do not assume liability for direct or indirect damages that may result from the use of this report. The responsibility for implementing and maintaining effective security measures lies with the assessed organization.`,
    FR: `<p>Ce rapport est basé sur les informations, données et preuves rendues disponibles pendant le processus d’évaluation. Bien que tous les efforts aient été faits pour fournir des résultats exacts et fiables, les constats et recommandations sont limités au périmètre de l’évaluation et à la date de son exécution.<p>Le rapport ne doit pas être considéré comme une garantie contre les risques ou incidents futurs. Les menaces de sécurité évoluent constamment, et une surveillance, amélioration et adaptation continues restent essentielles.<p>L’évaluateur et l’organisation d’évaluation n’assument aucune responsabilité pour les dommages directs ou indirects pouvant résulter de l’utilisation de ce rapport. La responsabilité de mettre en œuvre et de maintenir des mesures de sécurité efficaces incombe à l’organisation évaluée.`,
    DE: `<p>Dieser Bericht basiert auf den während des Bewertungsprozesses verfügbaren Informationen, Daten und Beweisen. Obwohl alle Anstrengungen unternommen wurden, genaue und zuverlässige Ergebnisse zu liefern, sind die Feststellungen und Empfehlungen auf den Umfang der Bewertung und den Zeitpunkt ihrer Durchführung begrenzt.<p>Der Bericht sollte nicht als Garantie gegen zukünftige Risiken oder Vorfälle betrachtet werden. Sicherheitsbedrohungen entwickeln sich ständig weiter, und kontinuierliche Überwachung, Verbesserung und Anpassung bleiben unerlässlich.<p>Der Prüfer und die prüfende Organisation übernehmen keine Haftung für direkte oder indirekte Schäden, die durch die Nutzung dieses Berichts entstehen können. Die Verantwortung für die Umsetzung und Aufrechterhaltung wirksamer Sicherheitsmaßnahmen liegt bei der geprüften Organisation.`
  },
...
const buildToc = (templateStructure, lang) => {
  ...
  const order = [
    DICT.INTRO[lang],
    DICT.ABOUT_CONSULTING[lang],
    DICT.ABOUT_AUDITED[lang],
    DICT.PREFACE[lang],
    DICT.DISCLAIMER[lang],
    DICT.EXEC_SUMMARY[lang],
    DICT.EXAM_ENV[lang],
    DICT.FINDINGS[lang],
    DICT.HANDOVER[lang],
    DICT.THANK_YOU[lang]
  ...
  order.forEach((title, idx) => {
    html += `<li><a href="#sec-${idx}">${escapeHtml(title)}</a></li>`;
  });
  html += '</ul>';
  return html;
};
...
<head>
  ...
  <style>
    @page{margin:0.35in; @bottom-center{content:counter(page)"/"counter(pages); font-family:Arial,sans-serif; font-size:10pt; color:#666;}}
    body{font-family:Arial,sans-serif; font-size:14pt; color:#2c3e50; margin:0; -webkit-print-color-adjust:exact;}
    .container{padding:0.35in; box-sizing:border-box;text-align:justify;}
