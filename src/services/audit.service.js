...
class AuditService {
  ...
  async createAuditInstance(
    ...
    // 🔥 TRANSLATE BEFORE SNAPSHOT
    const translatedTemplate = await translateAuditTemplate(template, lang);

    const audit = new AuditInstance({
      company: companyId,
      template: templateId,
      assignedTo: assignedToUserId,
      createdBy: createdByUserId,
      ...
      templateStructureSnapshot: JSON.parse(
        JSON.stringify(translatedTemplate.sections)
      ),
    });
  ...
  async getAuditInstance(auditId, userId, role, lang) {
    const audit = await AuditInstance.findById(auditId)
      .populate('company', 'name')
      .populate('createdBy', 'firstName lastName')
      .populate('assignedTo', 'firstName lastName')
    ...
    // 🔥 TRANSLATE SNAPSHOT PER REQUEST
    const translatedSections = await translateAuditTemplate(
      { sections: audit.templateStructureSnapshot },
      lang
    ).then(t => t.sections);
  ...
  async updateAuditInstance(auditId, updates, userId, role, lang) {
    const audit = await AuditInstance.findById(auditId);
    if (!audit) throw new Error('AUDIT_INSTANCE_NOT_FOUND');
    ...
    const translatedSections = await translateAuditTemplate(
      { sections: updated.templateStructureSnapshot },
      lang
    ).then(t => t.sections);
