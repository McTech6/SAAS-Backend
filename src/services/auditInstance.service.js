...
import generateReportHtml from '../utils/reportGenerator.js'; // Import the HTML generator
...
class AuditInstanceService {
    ...
    async generateReport(auditInstanceId, requestingUser) {
        // Fetch the audit instance with all necessary populated data
        const auditInstance = await AuditInstance.findById(auditInstanceId)
            .populate('company') // Populate company details
            .populate('createdBy', 'firstName lastName email') // Populate creator details
            .populate('template', 'name version'); // Populate template name/version
        ...
        // Ensure user is authorized to view this audit instance for report generation
        // Re-using the access control logic from getAuditInstanceById
        const userHasAccess = await this.getAuditInstanceById(auditInstanceId, requestingUser);
        if (!userHasAccess) {
             // This case should ideally be caught by getAuditInstanceById throwing, but as a safeguard
        ...
        try {
            ...
            const htmlContent = generateReportHtml(auditInstance);
            ...
            const pdfBuffer = await page.pdf({
                ...
                headerTemplate: `<div style="font-size: 9pt; color: #777; text-align: center; width: 100%; padding: 10px 0;"></div>`, // Empty header
                footerTemplate: `
                    <div style="font-size: 9pt; color: #777; text-align: center; width: 100%; padding: 10px 0;">
